...
/**
 * Serviço responsável pelo ciclo de vida da Ordem de Serviço (OS).
 *
...
public class ServicoService {
    ...
    public ServicoResponse salvarComoRascunho(Long id, RascunharServicoRequest request) {
        ...
        var blocoRealizado = montarBlocoRealizado(
                servico.getBlocoRealizado(),
                request.horasRealizadas(),
                request.custoReal(),
                request.valorFaturado(),
                request.dataRealEntrega(),
                request.houveRetrabalho(),
                request.houveMudancaEscopo()
        );

        var blocoAprendizado = licaoService.montarBlocoAprendizado(
                servico.getBlocoAprendizado(),
                request.causaDesvioId(),
                request.licaoAprendida(),
                request.assuntosRelacionadosIds(),
                request.restrito(),
                StatusLicao.RASCUNHO,
                null,
                false
        );
    ...
    public ServicoResponse concluirServico(Long id, ConcluirServicoRequest request) {
        ...
        licaoService.notificarNovaLicao(servico, false);

        return mapper.toResponse(servico);
    }

    private BlocoRealizado montarBlocoRealizado(
            BlocoRealizado blocoExistente,
        ...
        var bloco = blocoExistente != null ? blocoExistente : new BlocoRealizado();
        bloco.setHorasRealizadas(horasRealizadas);
        bloco.setCustoReal(custoReal);
        bloco.setValorFaturado(valorFaturado);
        bloco.setDataRealEntrega(dataRealEntrega);
        bloco.setHouveRetrabalho(houveRetrabalho);
        bloco.setHouveMudancaEscopo(houveMudancaEscopo);
        return bloco;
    }
}
